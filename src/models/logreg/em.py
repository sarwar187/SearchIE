...
from InformativePriorLogisticRegression import InformativePriorLogisticRegression
...
def train_informative_logreg(_X, _Q, w0, b0, C=5):
    ...
    model = InformativePriorLogisticRegression(w0, b0, C)
...
def go_em(output_file, X, X_test, Y_test, E, Epos, toprint_test, prior_w0, C=0.1, Niter=2, trainfn=train_informative_logreg):
    ...
    xx = trainfn(X.todense(), init_preds, prior_w0, 0, C=5)
    #cur_preds = xx['preds']
    #save#5zero_prior.run the sent-level predictions (to be used in eval2)
    #output_file = 'src/models/preds/em0.json'
    ...
    for itr in xrange(1, Niter+1):
        ...
        # M-step
        xx = trainfn(X.todense(), Q, prior_w0, 0, C=5)
    ...
    #output_file = 'src/models/preds/em{0}.json'.format(itr)
    #print output_file
    save_sent_level_score_ordered(X_test, toprint_test, output_file, xx['model'])
...
def save_sent_level_score_ordered(X, toprint_test, w, model):
    probs = model.predict_proba(X)
    #print len(toprint_test)
    #print len(probs)
    assert len(toprint_test) == len(probs)
    for i, yprob in enumerate(probs):
        obj = toprint_test[i]
...
w0 = np.asarray([0,0])
b0 = np.asarray([0])
ilp = InformativePriorLogisticRegression(w0, 0, 1)