...
def main():
    ...
    output_file = open("feature_importance_output_file.tsv", "w")
    ...
    query_terms = np.asarray([5, 10, 20, 40, 70, 110, 160, 220, 290, 350, 400])
    number_of_files = 1
    query_terms = [290]
    res = np.zeros((4, len(query_terms)))
    ...
    for idx, file in enumerate(onlyfiles[:number_of_files]):
        ...
        for index in np.arange(len(query_terms)):
        ...
        output_file.write(str(query_terms) + "\n")
