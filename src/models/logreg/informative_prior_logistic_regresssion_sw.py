...
class InformativePriorLogisticRegressionWeight(object):
...
def main():
    ...
    for index, l in enumerate(lambdas):
        for index1, num_training in np.ndenumerate(training_data_cases):
            ...
            cls = InformativePriorLogisticRegressionWeight(w_prior, b_prior, l)
            cls.fit(temp_train_X, temp_train_y, np.ones(temp_train_X.shape[0]))
