...
'''
this src is for ensuring that the 'revs' from Yoon Kims preproc match the order
...
def build_data_cv(data_folder, cv=10, clean_string=False):
    """
    Loads data. cv=10 is from yoon kim's original src. We don't use it.
    """
    revs = []
...
if __name__=="__main__":
    ...
    for i in range(len(datasets[1])):
        ...
        if i > 0:
            datasets[1][i] = None # save#5zero_prior.run space for pickle
